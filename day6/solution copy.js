...
function goForward(row, col){
  if (dir === 'up') {
    if(inputorder[row-1] && 
      ...
      if (pos.includes(row+" "+col+1)) {
        isOutofbound(row, col+1, 'right');
      }
      row--;
      console.log(row, col);
  ...
  if (dir === 'right') {
    ...
    inputorder[row][col+1] !== '#') {
      console.log("going " + dir);
      if (pos.includes(row+1+" "+col)) {
        isOutofbound(row+1, col, 'down');
      }
      col++;
      console.log(row, col);
      if (pos.includes(row+" "+col)) {
  ...
  if (dir === 'down') {
    ...
    inputorder[row+1][col] !== '#') {
      console.log("going " + dir);
      // check if position on their right that is "up" already visited
      if (pos.includes(row+" "+col-1)) {
        isOutofbound(row, col-1, 'left');
      }
      row++;
      console.log(row, col);
      if (pos.includes(row+" "+col)) {
...
function isOutofbound(row, col, dir){
  let key = row + " " + col;
  if(seenmap.get(key)){
    return;
  }
  while(inputorder[row][col] === '#' ||
    row < 0 || row > inputorder.length-1 ||
    col < 0 || col > inputorder[row].length-1
  ){
    if(dir === 'up'){
      row--;
    }
  ...
  // return if it was out of bound or blocked
  if(row < 0 || row > inputorder.length-1 ||
    col < 0 || col > inputorder[row].length-1){
      return true;
  }
  else {
